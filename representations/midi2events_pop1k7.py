...
def event2lead_full(events, keyname, relative_chord, relative_melody):
    ...
    for s in range(len(full_start)):
        ...
        for b in range(16):
            if b in beat_seq:
                events.append(create_event('Beat', b))
                events += beat_seq[b]
        final_events += events
...
if __name__ == '__main__':
    ...
    if event_type == 'lead2full':
        new_dir = 'events/emo_dstg/pop1k7_lead2full_{}/events'.format(representation)
    elif event_type == 'full':
        new_dir = 'events/emo_dstg/pop1k7_fullsong_{}/events'.format(representation)
