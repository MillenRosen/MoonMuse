...
def midi2corpus(midi_obj):
    ...
    for key in instr_notes.keys():
        ...
        for note in notes:
            ...
            if ntick_duration == 0:
                continue
            # note.quantized_duration = ntick_duration
            note.quantized_duration = ntick_duration
    ...
    for bar_step in range(0, global_end, BAR_RESOL):
        ...
        for timing in range(bar_step, bar_step + BAR_RESOL, TICK_RESOL):
            ...
            if len(t_chords):
                ...
                events.append(create_event('Chord_Root', root))
                events.append(create_event('Chord_Quality', quality))
                ...
                for note in t_notes:
                    if relative_melody:
                        ...
                        events.extend([
                            ...
                            create_event('Note_Duration', note.quantized_duration),
            ...
            if len(t_chords):
                ...
                if relative_melody and root != 'None':
                    if global_key in MAJOR_KEY:
                        ...
                        root = minorDegree2roman[int(root)]
                lead_events.append(create_event('Chord_Root', root))
                lead_events.append(create_event('Chord_Quality', quality))
            ...
            if len(t_chords):
                ...
                if relative_melody and root not in ['None', 'Conti']:
                    if global_key in MAJOR_KEY:
                        ...
                        root = minorDegree2roman[int(root)]
                full_events.append(create_event('Chord_Root', root))
                full_events.append(create_event('Chord_Quality', quality))
